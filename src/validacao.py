...
def valida_estrutura(df: pd.DataFrame) -> list[str]:
    ...
    Verifica se o DataFrame contém todas as colunas esperadas. Retorna a lista
    de colunas ausentes. Uma lista vazia significa que a estrutura está correta.
    """
...
def valida_campos_obrigatorios(df: pd.DataFrame) -> list[dict]:
    ...
    (None, NaN ou texto em branco). Retorna a lista de ocorrências encontradas.
    O número da linha usa a numeração da planilha (começa em 2, já que a linha 1
    é o cabeçalho).
    """
